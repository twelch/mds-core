...
async function writeDevice(device: Device) {
  if (env.SINK) {
    return write('ai.lacuna.device', JSON.stringify(device))
  }
...
async function writeEvent(event: VehicleEvent) {
  if (env.SINK) {
    return write('ai.lacuna.event', JSON.stringify(event))
  }
...
async function writeTelemetry(telemetry: Telemetry[]) {
  if (env.SINK) {
    return Promise.all(telemetry.map(item => write('ai.lacuna.telemetry', JSON.stringify(item))))
  }
  const start = now()
